...
#     decompiled_version = make_paths(version, side, removal_bool, force, forceno)
...
def main(mcversion: str, quiet=False, clean=True, decompiler="cfr", side="server", force=False, forceno=True, nauto=False, download_mapping=True, remap_mapping=True, download_jar=True, remap_jar=True, delete_dep=True, decompile=True):
    check_java()
    snapshot, latest = get_latest_version()
    if snapshot is None or latest is None:
        print("Error getting latest versions, please refresh cache")
        sys.exit(1)
    # TODO: all ifs including use_flags are fucking usless, refactor
    use_flags = True
